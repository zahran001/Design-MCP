...
import { Box, Container, Heading, HStack, Link, Spinner, Stack, Text } from '@chakra-ui/react';
...
// Minimal hash routing (no router dep): #/engineering -> the engineering
// showcase, anything else -> the generator. Shareable via the URL hash.
type Route = 'generator' | 'engineering';
const parseHash = (): Route =>
  window.location.hash.replace(/^#\/?/, '') === 'engineering' ? 'engineering' : 'generator';
...
function TopNav({ route }: { route: Route }) {
  ...
  return (
    <HStack justify="space-between">
      <Link href="#/" fontWeight="bold" textDecoration="none" _hover={{ textDecoration: 'none' }}>
        Spec to Component
      </Link>
      <HStack gap={5}>
        {item('#/', 'Generator', route === 'generator')}
        {item('#/engineering', 'Engineering', route === 'engineering')}
      </HStack>
    </HStack>
  );
}
              ...
              {!loading && !report && !error && (
                ...
                >
                  <Text color="fg.muted" textAlign="center" fontSize="sm">
                    Your generated component will appear here: live preview, validated code, and the
                    docs it was grounded in.
                  </Text>
                </Box>
              )}
                    ...
                    report.context[0].score < WEAK_GROUNDING_THRESHOLD && (
                      ...
                      >
                        ...
                        <Text color="orange.fg" fontSize="sm" mt={1}>
                          ...
                          the output relies on the model's own knowledge rather than the v3 docs — trust
                          the report badges over the result.
                        </Text>
                      </Box>
                    )}
